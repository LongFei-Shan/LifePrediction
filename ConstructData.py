...
def loadTrainData():
    # 读取训练数据
    dir = r"D:\LongFei-Shan\数据集\bearing\PHM\Full_Test_Set"
    ...
    trainDataLifeTime = {"Bearing1_3": 300.5, "Bearing1_4": 189.6, "Bearing1_5": 383.5, "Bearing1_6": 383.5,
                         "Bearing1_7": 250.18, "Bearing2_3": 200.16, "Bearing2_4": 101.83, "Bearing2_5": 333.5,
                         "Bearing2_6": 95.18, "Bearing2_7": 28.5, "Bearing3_3": 58.5}
    dirNames = os.listdir(dir)[10:11]
    trainData = {}
    trainLabel = {}
    # 读取每个文件最后两列数据
    sep=","
    for dirName in dirNames:
        if dirName == "Bearing1_4":
            sep = ";"
        fileNames = os.listdir(f"{dir}/{dirName}")
        data = pd.read_csv(f"{dir}/{dirName}/{fileNames[0]}", sep=sep, encoding="utf-8").values
        for fileName in tqdm.tqdm(fileNames[1:], desc=f"读取训练数据进度-{dirName}", ncols=100, file=sys.stdout):
            tempData = pd.read_csv(f"{dir}/{dirName}/{fileName}", sep=sep, encoding="utf-8").values
...
if __name__ == "__main__":
    ...
    for dirName in tqdm.tqdm(dirNames, desc="特征选择进度", ncols=100, file=sys.stdout):
        ...
        faultLabel = np.sort(np.linspace(0, trainDataLifeTime[dirName], len(feature)))[::-1]
    ...
    allFeature = pd.read_csv(f"./Data/FeatureData/featureSmooth-{dirNames[0]}.csv", sep=",", encoding="utf-8").values[1:, :]
    ...
    for dirName in dirNames[1:]:
        tempfeature = pd.read_csv(f"./Data/FeatureData/featureSmooth-{dirName}.csv", sep=",", encoding="utf-8").values[1:, :]
