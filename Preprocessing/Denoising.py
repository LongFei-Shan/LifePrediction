...
class WaveletDenoising:
    def __init__(self, waveletName="db4", thresholdName="VisuShrink", thresholdFunctionName="soft", level=5, mode="symmetric"):
    ...
    def getThreshold(self, coeffs: np.ndarray) -> float:
        ...
        if self.thresholdName == "heursure":
            ...
            rish = []
            for i in range(len(f)):
                rish.append((len(coeffs) - 2 * i + np.sum(f[:i]) + (len(coeffs) - i) * f[len(coeffs) - i - 1]) / len(coeffs))
            indiex = np.argmin(rish)
    ...
    # 获取小波分解细节系数并输入到阈值函数中进行阈值处理，处理后进行小波重构
...
class EMDDenoising:
    def __init__(self, denoiseMethodName="EED", methodName="EMD", thresholdName="VisuShrink", T: Optional[np.ndarray] = None, max_imf: int = -1, progress: bool=False, thresholdACF=0.5, exceedThresholdNumber=5,
                 waveletName="db4", thresholdFunctionName="soft", level=5, mode="symmetric"):
        """
        :param denoiseMethodName: 信号分解方法: EED, ECD
    ...
    def signalDecompose(self, signal: np.ndarray) -> np.ndarray:
        decomposeMethod = {"EMD":lambda:PyEMD.EMD().emd(S=signal, T=self.T, max_imf=self.max_imf),
                           "EEMD":lambda:PyEMD.EEMD().eemd(S=signal, T=self.T, max_imf=self.max_imf, progress=self.progress),
                           "CEEMDAN":lambda:PyEMD.CEEMDAN().ceemdan(S=signal, T=self.T, max_imf=self.max_imf, progress=self.progress)}
    ...
    def ECD(self, signal: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(acf)):
            if self.judgeIMFIsNoise(acf[i]):
                imfs[i, :] = WaveletDenoising(waveletName=self.waveletName, thresholdName=self.thresholdName, thresholdFunctionName=self.thresholdFunctionName
